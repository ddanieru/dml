...
!> A type within the values defined
type, public :: dml_const_t
    ! -----------------------------
    ! MATH
    ! -----------------------------
    real(kind=wp) :: pi      = 3.141592653589793d0  ! \pi
    real(kind=wp) :: e_euler = 2.718281828459045d0  ! napier or euler
    real(kind=wp) :: gama   = 0.577215664901532d0  ! euler-mascheroni
    real(kind=wp) :: phi     = 1.618033988749894d0  ! golden ratio
    ! -----------------------------
    ! UNIVERSAL
    ! -----------------------------
    real(kind=wp) :: mu_0      = 12.566370614d-7    !(exact)                  N A^-2
    real(kind=wp) :: epsilon_0 = 8.854187817d-12    !(exact)                  F m^-1
    real(kind=wp) :: c_0       = 299792458d0        !(exact)                  m s^-1
    real(kind=wp) :: gravity   = 9.80665d0          !(exact)                  m s^-2
    real(kind=wp) :: atm       = 101325d0           !(exact)                  Pa
    real(kind=wp) :: Z_0       = 376.730313461d0    !(exact)                  ohm
    real(kind=wp) :: G         = 6.67384d-11        !0.000 80 e-11            m^3 kg^-1 s^-2
    real(kind=wp) :: h         = 6.62606957d-34     !0.000 000 29 e-34        J s
    real(kind=wp) :: hbar      = 1.054571726d-34    !0.000 000 047 e-34       J s
    real(kind=wp) :: l_p       = 1.616199d-35       !0.000 097 e-35           m
    real(kind=wp) :: m_p       = 2.17651d-8         !0.000 13 e-8             kg
    real(kind=wp) :: Temp_p    = 1.416833d32        !0.000 085 e32            K
    real(kind=wp) :: t_p       = 5.39106d-44        !0.000 32 e-44            s
    ! -----------------------------
    ! PHYSICO-CHEMICAL
    ! -----------------------------
    real(kind=wp) :: N_A = 6.02214129d23            !0.000 000 27 e23         mol^-1
    real(kind=wp) :: m_u = 1.660538921d-27          !0.000 000 073 e-27       kg
    real(kind=wp) :: F   = 96485.3365d0             !0.0021                   C mol^-1
    real(kind=wp) :: R   = 8.3144621d0              !0.000 0075               J mol^-1 K^-1
    real(kind=wp) :: k_B = 1.3806488d-23            !0.000 0013 e-23          J K^-1
    real(kind=wp) :: V_m = 22.710953d-3             !0.000 021 e-3            m^3 mol^-1
    ! -----------------------------
    ! ATOMIC AND NUCLEAR
    ! -----------------------------
    real(kind=wp) :: m_e = 9.10938291d-31           !0.000 000 40 e-31        kg
    real(kind=wp) :: E_h = 4.35974434d-18           !0.000 000 19 e-18        J
    real(kind=wp) :: a_0 = 0.52917721092d-10        !0.000 000 000 17 e-10    m
    ! -----------------------------
    ! NON-SI UNITS
    ! -----------------------------
    !   natural units (n.u.)
    ...
    !   atomic units (a.u.)
    ! -----------------------------
    real(kind=wp) :: e = 1.602176565e-19        !0.000 000 035 e-19       C
end type dml_const_t

type(dml_const_t), public, save :: dml_const !> dml_const_t type variable
